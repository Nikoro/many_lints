...
This rule is in **no preset**, and deliberately so. Measured against a production Flutter app that already follows a consistent house style, it still reported 227 members — every one a real deviation, none of them a bug. That is taste, not correctness, so it stays opt-in by name and the order is yours to declare.
