...
## Classes that already inherit `@immutable` are never reported

Widget names ending in `State` are idiomatic Flutter — `EmptyState`, `ErrorState`, `LoadingState` describe what is rendered, not a state object. Flutter annotates `Widget` `@immutable`, so every `StatelessWidget` already has the annotation this rule asks for, and reporting one would be asking for something it inherits.

The check is on the supertypes, so it is not Flutter-specific: any base class annotated `@immutable` passes it down to its subclasses.
