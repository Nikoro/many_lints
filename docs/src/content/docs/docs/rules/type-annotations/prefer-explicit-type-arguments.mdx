...
Assuming `showDialog` is in the configured `methods` list, it infers its type
argument from what the builder pops — so a cancel path that pops without a value
makes the whole call `Future<Null>`.
