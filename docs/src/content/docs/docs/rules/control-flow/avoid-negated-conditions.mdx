...
### `!=` against a real value counts as a negation

Comparing against anything other than `null` or `0` reads as "not this one", so the `else` is again the positive branch:
...
Set `report_not_equal: false` to leave every `!=` alone and report only `!`:
...
`!= null` and `!= 0` are never reported either way.
