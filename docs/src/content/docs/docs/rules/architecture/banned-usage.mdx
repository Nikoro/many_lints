...
### Worked example: scope a layer, then carve out one exception
...
rule) skips paths for the **whole rule**. Together they express "banned across
this layer, except in the one place that legitimately does it" — which is
otherwise a shell script:
...
Writing to stdout is now an error everywhere under `lib/` except the one
renderer that owns the terminal. Domain code returns a value; a single layer
decides how it is displayed, which is what makes the output testable.
