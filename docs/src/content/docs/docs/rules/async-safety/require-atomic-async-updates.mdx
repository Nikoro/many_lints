...
A compound assignment is the same hazard written shorter, because `+=` reads the field before writing it. It is reported when the field was also read before the await — here by the log line:
...
Drop that log line and nothing is reported: with no read before the await there is no stale value for the write to be measured against. See [Known limitations](#known-limitations).
...
**The field must be read before the await.** That read is what the write is compared against, so a body that only writes after the await reports nothing — including a compound assignment:

```dart
Future<void> increment() async {
  await _repository.save();
  _value += 1;   // not reported: nothing read `_value` before the await
}
```

This is the most common reason a hazard you expected to see is missing.
...
Closures are excluded from the dependency check as well as from the write check. A field named inside the callback a write installs does not make that write depend on the pre-await read, so the ordinary cancel-then-reassign shape is left alone:

```dart
await Clipboard.setData(data);
if (!mounted) return;
_timer?.cancel();
_timer = Timer(d, () => _timer?.cancel());   // not reported
```
