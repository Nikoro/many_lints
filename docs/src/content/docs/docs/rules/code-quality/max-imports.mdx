...
`export` directives are not counted by default — a barrel file is exports by definition.
...
import 'dart:developer'; // LINT: 6 imports exceeds the budget of 5
...
Split the file so each part imports what it needs:
