...
Add `archived` to the enum and this function silently returns `''` for it. The
same blind spot applies to comparisons joined by `||`, and to a membership test
over a set literal:

```dart
bool isOpen(Status status) {
...
bool isOpen(Status status) => switch (status) {
