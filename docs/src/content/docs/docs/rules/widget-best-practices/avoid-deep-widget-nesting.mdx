...
Extracting a subtree into a named widget removes a whole level and gives the part a name at the same time — which is why this rule pairs naturally with [`prefer_single_widget_per_file`](/many_lints/docs/rules/widget-best-practices/prefer-single-widget-per-file/).
