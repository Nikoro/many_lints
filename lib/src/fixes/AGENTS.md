...
| Fix built from user configuration | [match_pattern_fix.dart](match_pattern_fix.dart) | Applies a `replace:` template the project wrote. Four guard rails the hand-written fixes do not need: opt-in per entry, **parse the expansion before offering it** (`parseString` inside a trivial wrapper), `singleLocation` so `dart fix --apply` cannot sweep a regex across a project, and drop a malformed entry rather than throw. Re-resolve config with `ResolvedRuleConfig.forPath` (a fix holds no rule instance) and use the built-in `coveringNode` rather than hand-walking offsets. Assert the withheld-fix rails with `FixHarness.expectNoFix` |
