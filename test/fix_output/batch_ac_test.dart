...
''';
  ...
  group('avoid_unnecessary_overrides_in_state', () {
    test('removes a dispose() override that only calls super', () async {
      final fixed = await harness.applyFix(
        r'''
import 'package:flutter/flutter.dart';
...
''',
      ...
      expect(fixed, isNot(contains('void dispose()')));
      expect(
        fixed,
        contains('Widget build(Object context) => const Widget();'),
      );
    });

    test('removes an initState() override with expression body', () async {
      final fixed = await harness.applyFix(
        r'''
import 'package:flutter/flutter.dart';
...
''',
      ...
      expect(fixed, isNot(contains('initState')));
      expect(
        fixed,
        contains('Widget build(Object context) => const Widget();'),
      );
